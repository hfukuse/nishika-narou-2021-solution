...
class Config:
    ...
    pos_dir = js["pos_dir"]
    output_dir = js["result_dir"]
    model_dir = js["models_dir"] + "/" + js["n126"]["model_dir"]
...
df1 = pd.read_csv(Config.n117_inf + "/valid.csv")
df2 = pd.read_csv(Config.n117_inf + "/test_submission.csv")
df2.columns = df1.columns
df = pd.concat([df1, df2]).reset_index(drop=True)
df.columns = ["ncode", "mr_proba_0", "mr_proba_1", "mr_proba_2", "mr_proba_3", "mr_proba_4"]
df["mr_proba"] = df["mr_proba_0"] * 5000 + df["mr_proba_1"] * 500 + df["mr_proba_2"] * 50 + df["mr_proba_3"] * 5 + df[
    "mr_proba_4"] * 0
concat_df = pd.merge(concat_df, df)
num_cols += ["mr_proba"]
...
for i in range(5):
    ...
    acc.append(accuracy)
    score.append(model.best_score_["validation"]["MultiClass"])
print("**acc**")
